...
- So finally this method involves guessing $\alpha$ which is called the **_shooting parameter_**, solving the IVP and then using a root-finding method to find a better estimate for alpha using the above condition.
...
# ╔═╡ fea0a209-afba-4dc2-bcfb-6e27e2160bdc
html"<br><br>"

# ╔═╡ 2d7bae04-f67c-419c-9312-cbf698211711
md"## Linear Boundary Value Problems

- Linear BVPs are problems that can be written as:

$y''(x) + P(x)y'(x) + Q(x)y(x) = R(x),\quad y(x_0)=a,\quad y(x_N)=b$

- If $R(x) = 0$, then such problems are called _Homogeneous Boundary Value Problems_.

- Such problems are linear because linear combinations of any two solutions is a also a solution.

- In order to solve the linear boundary value problems, consider the following initial value problems:

  $u''(x) + P(x)u'(x) + Q(x)u(x) = R(x),\quad u(x_0)=a,\quad u'(x_0)=0$
  $v''(x) + P(x)v'(x) + Q(x)v(x) = 0,\quad v(x_0)=0,\quad v'(x_0)=\theta s$

  Then define $w(x) = u(x) + \theta v(x)$, here it can be seen that due to linearity $w(x)$ is a solution of the sum of the two initial value problems with:
  
  $w(x_0)=a,\quad w'(x_0)=\theta s$

  If we impose another condition that $w(x_N) = u(x_N) + \theta v(x_N) = b$ then w(x) is also the solution of our original BVP. This means that:
  $\theta = \cfrac{b-u(x_N)}{v(x_N)}$.

- Hence, the solution of the linear boundary value problem can be written as:

  $y(x) = u(x) + \cfrac{b-u(x_N)}{v(x_N)}v(x)$

  Where, $u(x)$ and $v(x)$ are the two initial value problem given above. This is an improvement on the method for solving a general BVP as it does not involves guessing the initial conditions and iterating until a solution is found.

- There is also a requirement that $v(x_N)\ne 0$ for this method to work. This is satisfied when $Q(x)\le 0$ for $x\in[x_0, x_N]$ (from Strum-Liouville theory).

- Now, it can be seen that the time-independent Schrödinger's equation is a linear eigenvalue problem, particularly a *homogeneous BVP*:

  $\psi''(x) + \frac{2m}{\hbar^2}[E-V(x)]\psi(x)=0$

  According to the last condition, this method can be applied on the Schrödinger's equation if $E\le V(x)$. Which corresponds to the _bound state solutions_.

- In case of **homogeneous BVPs**, if $y(x_0)=0$ then the first of the two initial value problems has a trivial solution, i.e. $u(x)=0$. Hence, the solution of the BVP is given by:

  $y(x) = u(x) + \cfrac{b-u(x_N)}{v(x_N)}v(x) = \cfrac{b}{v(x_N)}v(x)$

  This is just a solution of initial value problem multiplied by a constant factor! Hence, such a _HBVP_ with $y(x_0)=0$ is equivalent to an IVP. Here $s$ is a _free shooting parameter_ as it's choice doesn't matter and any non-zero value will work. **But in practice it is better to choose small values for $s$ to prevent overflow errors due to large values of floating point numbers that will be reached for large $s$.** 
"
...
md"""
## Solving the Schrödinger's Equation using Shooting Method

- The solutions of the Schrödinger's Equation have to be noramalizable, so for this the following condition has to be imposed:

  $\psi(x)\rightarrow 0\ \text{as}\ x\rightarrow\pm\infty$

- In a numerical algorithm, instead of having boundary conditions at infinity, choose a finite domain $[-a,a]$ such that $V(\pm a)$ is sufficiently large such that, $\psi(\pm a) \approx 0$. 

!!! tip "The Numerical Solution"

- **Discretization**:
  1. Discretize the grid:
  $x_i\in[x_0, x_{N-1}],\ \Delta x = \cfrac{x_{N-1}-x_0}{N-1},\ x_{i+1} = x_i + \Delta x$
  
  2. Using second derivative central finite difference:
  $\psi_j'' = \cfrac{\psi_{j+1}- 2\psi_j + \psi_{j-1}}{\Delta x^2}$
  
  3. Discretized Schrödinger's equation for $j=1,\cdots,N-2$:
  $\cfrac{-\hbar^2}{2m}\cfrac{\psi_{j+1}- 2\psi_j + \psi_{j-1}}{\Delta x^2} + V_j\psi_j = E\psi_j$
  $\implies \psi_{j+1} = 2\bigg[\cfrac{m(\Delta x)^2}{\hbar^2}(V_j-E)+1\bigg]\psi_j-\psi_{j-1}$

  4. For initial Conditions we have $x_0 = -a$, $x_{N-1}=a$ and $\psi(\pm a)\approx 0$. Now as the solutions are independent of the shooting parameter we can take:
  $\psi_0=0,\quad \psi_1 = \psi'_0\Delta x + \psi_0 = \psi'_0\Delta x = s$

- **Normalization**: It will be extreamely rare that the solutions that are found using the numerical procedure will be normalized. So, in order to normalize the solutions, calculate using some method of numerical integration (like Simpson's Rule):

  $A = \int_{-\infty}^{\infty} |\psi(x)|^2 dx$

  And then multiply the solution by a factor of $\cfrac{1}{\sqrt{A}}$ to get the normalized solutions.

- **The Eigenvalues**: In the above approach it the energy eigenvalues $E$ are required, but if they are not known beforehand:
  1. Guess a value for $E^{(0)}$.
  2. Use the numerical method to solve for eigenfunctions $\psi$.
  3. At the grid point $x_{N-1}=a$, use a root finding method on the following equation to find a better approximation of $E^{(k)}$ until convergence for $k=0,1,2,\cdots$
